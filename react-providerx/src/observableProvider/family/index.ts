...
export class ObservableProviderFamily<
  T,
  P
> extends BaseObservableProviderFamily<T, P> {
  _store: any = {}
  observableCreator
  constructor(
    observableCreator: (ref: ProviderReference, param: P) => Observable<T>
  ) {
    super(observableCreator)
    this.observableCreator = observableCreator
  }
  static autoDispose<ST, SP>(
    observableCreator: (ref: ProviderReference, param: SP) => Observable<ST>
  ): AutoDisposeObservableProviderFamily<ST, SP> {
  ...
  pass(param: P): ObservableProvider<T> {
    ...
    if (retrievedProvider === undefined || retrievedProvider === null) {
      this._store[param] = new ObservableProvider((ref) => {
